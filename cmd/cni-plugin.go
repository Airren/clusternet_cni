...
func (p *plugin) RunPodSandbox(pod *api.PodSandbox) error {
	...
	cmdStr := "ls"
	err := Exec(cmdStr, os.Stdout, "-al", "/run/netns")
	fmt.Println(err)

	//fmt.Println(">>>>>>>>> check inode id")
	//cmdStr = "stat"
	//err = Exec(cmdStr, os.Stdout, networkNamespacePath)
	//fmt.Println(err)

	//networkNamespace := filepath.Base(networkNamespacePath)

	return AddNetworkInterface(networkNamespacePath)
	// err = Exec("/cnitool", os.Stdout, "add", "cbr1", networkNamespacePath)
	// fmt.Println("Command: ", fmt.Sprintf("CNI_PATH=/opt/cni/bin CNI_IFNAME=eth99 /cnitool %s %s %s", "add", "cbr1", networkNamespacePath))
	// return nil
...
func main() {
	...
	flag.StringVar(&cniName, "cniname", "", "comma-separated list of events to subscribe for")
	...
	if cniName == "" {
		fmt.Println("please specify a cni name through -cniname cbr1...this defined in the cni config file")
		cniName = "cbr1"
	}
	fmt.Printf(" the cni config now is %s", cniName)
	if cfg.LogFile != "" {
		f, err := os.OpenFile(cfg.LogFile, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
		if err != nil {
